...
def visualize_detections(image_path, output_path, detections, labels=[], iou_threshold=0.5):
    ...
    for d in detections:
        ...
        draw_keypoint = keypoints if keypoints else None
        if draw_keypoint:
            keypoint_color = "red"  # Color for the keypoint
            keypoint_radius = 5  # Radius for the keypoint circle
            draw.ellipse((draw_keypoint[0] - keypoint_radius, draw_keypoint[1] - keypoint_radius,
                          draw_keypoint[0] + keypoint_radius, draw_keypoint[1] + keypoint_radius),
