...
class DET2GraphSurgeon:
    def __init__(self, saved_model_path, config_file, weights):
        ...
        self.ROI_KEYPOINT_HEAD_NUM_KEYPOINTS  =  self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.NUM_KEYPOINTS
        ...
        self.keypoint_conv_layers = 8

        # self.ROI_KEYPOINT_HEAD_POOLER_RESOLUTION = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.POOLER_RESOLUTION
        # self.ROI_KEYPOINT_HEAD_POOLER_SAMPLING_RATIO = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.POOLER_SAMPLING_RATIO
        # self.ROI_KEYPOINT_HEAD_POOLER_TYPE = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.POOLER_TYPE

        self.ROI_KEYPOINT_HEAD_CONV_DIMS = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.CONV_DIMS
        self.ROI_KEYPOINT_HEAD_NORMALIZE_LOSS_BY_VISIBLE_KEYPOINTS = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.NORMALIZE_LOSS_BY_VISIBLE_KEYPOINTS
        self.ROI_KEYPOINT_HEAD_LOSS_WEIGHT = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.LOSS_WEIGHT
    ...
    def process_graph(self, anchors, first_nms_threshold=None, second_nms_threshold=None):
        ...
        def roi_heads(rpn_outputs, p2, p3, p4, p5, second_nms_threshold):
            ...
            print('ROIALIGNNNN', mask_pooler_reshape_node)
            ...
            print('this is the input to the keypoibt', mask_head_conv)
            ...
            classes_reshape_node = self.graph.op_with_const("Reshape", "", nms_outputs[3], classes_reshape_shape)

            # Assume keypoints are processed after the NMS step similar to how masks are processed
            keypoint_pooler_output = self.ROIAlign(
                nms_outputs[1], p2, p3, p4, p5,
                self.keypoint_ROIAlign_pooled_size,
                self.keypoint_ROIAlign_sampling_ratio,
                self.keypoint_ROIAlign_type,
                self.second_NMS_max_proposals, 'keypoint_pooler'
            )


            keypoint_pooler_shape = np.asarray([self.second_NMS_max_proposals*self.batch_size, self.fpn_out_channels, self.keypoint_ROIAlign_pooled_size, self.keypoint_ROIAlign_pooled_size], dtype=np.int64)


            keypoint_pooler_reshape = self.graph.op_with_const(
                "Reshape", "keypoint_pooler/reshape", keypoint_pooler_output, keypoint_pooler_shape
            )

            keypoint_head_conv = self.graph.find_node_by_op_name("Conv", "/roi_heads/keypoint_head/conv_fcn1/Conv")
            ...
            # Continue from where you connect the reshaped keypoint pooler output to the first convolutional layer in the keypoint head
            # Assume keypoint_pooler_reshape is already defined and connected
            # keypoint_output = keypoint_pooler_reshape[0]

            # # Pass through additional convolutional and ReLU layers
            # for i in range(1, self.keypoint_conv_layers+ 1):
            #     keypoint_output = self.graph.add_op("Conv",
            #                                     f"conv_fcn{i}",
            #                                     inputs=[keypoint_output],
            #                                     attrs={"kernel_shape": [3, 3], "pads": [1, 1, 1, 1], "strides": [1, 1]})
            #
            #     keypoint_output = self.graph.add_op("Relu", f"conv_fcn_relu{i}", inputs=[keypoint_output])
            #
            # # Upsampling through transpose convolution to increase spatial resolution for more precise keypoint localization
            keypoint_output = self.graph.find_node_by_op_name("ConvTranspose",
                                            "/roi_heads/keypoint_head/score_lowres/ConvTranspose")

            keypoint_last_conv_reshape_shape = np.asarray(
                [self.batch_size, self.second_NMS_max_proposals, self.keypoint_out_res ,
                 self.keypoint_out_res], dtype=np.int64)

            keypoint_final_graph_reshape_node = self.graph.op_with_const("Reshape", "keypoint_head/final_reshape",
                                                                keypoint_output.outputs[0], keypoint_last_conv_reshape_shape)

            keypoint_final_graph_reshape_node[0].dtype = np.float32
            keypoint_final_graph_reshape_node[0].name = "detection_keypoints"


            # print('there it is', keypoint_output)
            #
            # keypoint_pred_shape = np.asarray([self.second_NMS_max_proposals*self.batch_size, self.ROI_KEYPOINT_HEAD_NUM_KEYPOINTS, 3],
            #                              dtype=np.int64)
            #
            # keypoint_pred_final= self.graph.op_with_const("Reshape", "keypoint_pred/reshape", keypoint_output,
            #                                              keypoint_pred_shape)

            # # Bilinear interpolation to upscale the feature maps
            # keypoint_output = self.graph.add_op("Resize",
            #                                 "Resize",
            #                                 inputs=[keypoint_output],
            #                                 attrs={"scale": [1, 1, 2, 2], "mode": "linear"})
            #
            # # Applying sigmoid to convert the final output into a probability map
            # keypoint_probs = self.graph.add_op("Sigmoid", "keypoint_sigmoid", inputs=[keypoint_output])
            #
            # # Optionally, apply a threshold to filter out low-confidence keypoints
            # threshold_value = np.array([0.5], dtype=np.float32)  # Example threshold value
            # threshold_constant = gs.Constant(name="threshold_constant", values=threshold_value)
            # keypoint_thresholded = self.graph.add_op(
            #     op_type="Greater",
            #     name="keypoint_threshold",
            #     inputs=[keypoint_probs, threshold_constant],
            #     outputs=["keypoint_threshold_output"]
            # )
            #
            # # Reshape or adjust dimensions as needed for your application
            # # Example: Assuming the output needs to be (N, num_keypoints, 3) where 3 includes (x, y, score)
            # #num_keypoints = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.NUM_KEYPOINTS
            # num_keypoints = self.ROI_KEYPOINT_HEAD_NUM_KEYPOINTS
            # output_shape = [self.batch_size, num_keypoints, 3]  # Example shape
            # final_keypoints = self.graph.add_op("Reshape",
            #                                 "final_keypoints_reshape",
            #                                 inputs=[keypoint_thresholded],
            #                                 attrs={"shape": output_shape})

            #             for i in range(1, len(self.keypoint_conv_layers) + 1):
#                         keypoint_pooler_output = self.graph.op("Conv",
#                                                                f"keypoint_conv{i}",
#                                                                inputs=[keypoint_pooler_output],
#                                                                attrs={"kernel_shape": [3, 3], "pads": [1, 1, 1, 1], "strides": [1, 1]})
#
#                         keypoint_pooler_output = self.graph.op("Relu", f"keypoint_relu{i}", inputs=[keypoint_pooler_output])
#
# # Upsampling through transpose convolution
#             keypoint_pooler_output = self.graph.op("ConvTranspose",
#                                        "keypoint_deconv",
#                                        inputs=[keypoint_pooler_output],
#                                        attrs={"kernel_shape": [4, 4], "pads": [1, 1, 1, 1], "strides": [2, 2]})
#
#            # Bilinear interpolation to upscale the feature maps
#             keypoint_pooler_output = self.graph.op("Resize",
#                                        "keypoint_upsample",
#                                        inputs=[keypoint_pooler_output],
#                                        attrs={"scale": [1, 1, 2, 2], "mode": "linear"})
#
#             keypoint_probs = self.graph.op("Sigmoid", "keypoint_sigmoid", inputs=[keypoint_pooler_output])
#
# # Optionally, apply a threshold to filter out low-confidence keypoints
# # This step is optional and depends on your specific needs
#             threshold = 0.5  # Example threshold
#             keypoint_thresholded = self.graph.op("Greater", "keypoint_threshold",
#                                      inputs=[keypoint_probs, self.graph.constant(value=threshold, dtype=np.float32)])
#
# # Reshape or adjust dimensions as needed for your application
# # Example: Assuming the output needs to be (N, num_keypoints, 3) where 3 includes (x, y, score)
#             num_keypoints = self.det2_cfg.MODEL.ROI_KEYPOINT_HEAD.NUM_KEYPOINTS
#             output_shape = [self.batch_size, num_keypoints, 3]  # Example shape
#             final_keypoints = self.graph.op("Reshape", "final_keypoints_reshape",
#                                             inputs=[keypoint_thresholded],
#                                             attrs={"shape": output_shape})
#
#             # The final output of keypoint_pooler_output now needs to be formatted or processed depending on how it's consumed.
#             # Typically, this would involve reshaping or further processing steps such as applying sigmoid or softmax,
#             # depending on your exact architecture needs.
#
#             # Continue with any additional steps, e.g., combining with other outputs, applying additional layers, etc.
#
#
            ...
            # print('mask outputttt', last_conv.outputs[0])
            ...
            return nms_outputs, final_graph_reshape_node[0],   keypoint_output.outputs[0]
        ...
        box_head_outputs, mask_head_output,  keypoint_pred_final = roi_heads(rpn_outputs, p2, p3, p4, p5, second_nms_threshold)
        ...
        final_outputs = box_head_outputs + [mask_head_output,  keypoint_pred_final]
        #final_outputs = box_outputs + [mask_outputs, keypoint_outputs]
        self.graph.outputs = final_outputs  
        self.sanitize()
