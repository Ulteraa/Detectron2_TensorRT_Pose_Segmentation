...
class TensorRTInfer:
    ...
    def infer(self, batch, scales=None, nms_threshold=None):
        ...
        keypoints = []
      #  print('this is the length',   keypoint_heatmaps.shape)
        ...
        for i in range(self.batch_size):
            ...
            for n in range(int(nums[i])):
                # Select a mask.
                mask = masks[i][n]
                keypoint = keypoint_heatmaps[n,i]
                # print(keypoint.shape)
                x, y = np.unravel_index(np.argmax(keypoint), keypoint.shape)
            #    print(f'this is keypoint position in the order of (x, y),  {(y, x)}')

                # Calculate scaling values for bboxes.
                scale = self.inputs[0]['shape'][2]

            #    print('this is the shape of the matrix',  scale)
                scale /= scales[i]
                scale_y = scale
                scale_x = scale
            #    print(f'scal_x is {scale_x}, and the scale_y is {scale_y}')
            #    print(f'the dimension of the box before scale is {boxes[i][n]}')
              ...
              #  print(f'the dimension of the box after scale is {bbox}')


                scale_x_ = (bbox[3] - bbox[1]) / 28
                scale_y_ = (bbox[2] - bbox[0]) / 28

              #  print(f' these are scale keypoint to bounding boxes, scal_x_ is {scale_x_}, and the scale_y_ is {scale_y_}')

                # Calculate keypoint positions scaled to bbox and then translate to original image scale
                keypoint_x = bbox[1] + x * scale_x_
                keypoint_y = bbox[0] + y * scale_y_
            #    print(f' these are the output {[keypoint_x, keypoint_y]}')
              ...
              #  print('---------------------------------------------------------------------------------------')
                ...
                detections[i].append({
                    ...
                    'mask': mask,
                    'keypoints': [keypoint_y, keypoint_x] # Adjust scale as necessary
                })
...
def main(args):
    ...
    for batch, images, scales in batcher.get_batch():
        ...
        for i in range(len(images)):
           ...
           # print('the size of the image is', scales)
