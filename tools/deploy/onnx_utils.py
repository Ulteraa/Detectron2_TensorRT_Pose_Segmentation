...
@gs.Graph.register()
def add_op(self, op_type, name, inputs, outputs=None, **attrs):
    """
    Add a generic operation to the ONNX graph.
    
    :param op_type: Type of the operation (e.g., 'Conv', 'Relu')
    :param name: Name of the node
    :param inputs: List of input nodes
    :param outputs: List of output node names
    :param attrs: Attributes for the operation
    """
    outputs = outputs or [name + ":0"]
    node = gs.Node(op=op_type, name=name, attrs=attrs)
    node.inputs = inputs
    node.outputs = [gs.Variable(name=o, dtype=np.float32) for o in outputs]
