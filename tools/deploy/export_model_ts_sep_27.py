...
import detectron2.config as con
...
def setup_cfg(args):
    ...
    address_ = '/home/fariborz/detectron2/tools/deploy/train_200_batch'
    register_coco_instances("experiment_test", {}, os.path.join(address_, "batch_200.json"),
                            os.path.join(address_, "images_200"))

    metadata = MetadataCatalog.get("experiment_test")
    ...
    print(' we are reading from config')
    cfg.freeze()
    return cfg
...
def get_sample_inputs(args):
    ...
    if args.sample_image is None:
        ...
        print(' the size of the image in the first block of code is :', cfg.DATASETS.TEST[0])
        first_batch = next(iter(data_loader))
        return first_batch
    else:
        # get a sample data
        original_image = detection_utils.read_image(args.sample_image, format=cfg.INPUT.FORMAT)
        print(' the size of the image in the second block of code is :', original_image.shape )
        # Do same preprocessing as DefaultPredictor
        # aug = T.ResizeShortestEdge(
        #     [cfg.INPUT.MIN_SIZE_TEST, cfg.INPUT.MIN_SIZE_TEST], cfg.INPUT.MAX_SIZE_TEST
        # )
        aug = T.ResizeShortestEdge ( [540, 540], 960)
